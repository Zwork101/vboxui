...
class CreateModal(ModalScreen[Machine]):
    ...
    def compose(self) -> ComposeResult:
        with Container():
            ...
            with TabbedContent():
                with TabPane("Init", id="tab-init"):
                    with Vertical(classes="form-option"):
                        with Horizontal():
                            ...
                            yield Input(
                                self.form_data["name-input"],
                                id="name-input",
                                validators=[UniqueName(self._api)],
                                validate_on=["changed"],
                                placeholder="Unique Machine Name",
                            )
                    ...
                    with Vertical(classes="form-option"):
                        with Horizontal():
                            ...
                            with Horizontal(classes="surround"):
                                yield Input(
                                    self.form_data["parent-input"],
                                    id="parent-input",
                                    validators=[PathExists(target_directory=True)],
                                    validate_on=["changed"],
                                )
                                yield Button(
                                    "Select Directory",
                        ...
                        with Horizontal():
                            ...
                            with Horizontal(classes="surround"):
                                yield Input(
                                    ...
                                    id="iso-input",
                                    validators=[PathExists(target_directory=False)],
                                    validate_on=["changed"],
                                )
                                yield Button(
                                    "Select File", id="iso-btn", classes="filepicker"
                                )
                ...
                with TabPane("Hardware", disabled=True, id="tab-hardware"):
                    with Vertical(classes="form-option"):
                        with Horizontal():
                            yield Static(
                                f"Base Memory: {self.form_data['memory-input']} MB",
                                id="memory-target",
                            )
                            with Horizontal(classes="surround"):
                                yield Static("500")
                                yield Slider(
                                    min=500,
                                    max=self._max_memory,
                                    step=100,
                                    id="memory-input",
                                    value=self.form_data["memory-input"],
                                )
                    ...
                    with Vertical(classes="form-option"):
                        with Horizontal():
                            yield Static(
                                f"CPU Cores: {self.form_data['cpu-input']}",
                                id="cpu-target",
                            )
                            with Horizontal(classes="surround"):
                                yield Static("1")
                                yield Slider(
                                    min=1,
                                    max=self._max_cpu_cores,
                                    step=1,
                                    id="cpu-input",
                                    value=self.form_data["cpu-input"],
                                )
                ...
                with TabPane("Storage", disabled=True, id="tab-storage"):
                    with Vertical(classes="form-option"):
                        with Horizontal():
                            ...
                            with Horizontal(classes="surround"):
                                yield Input(
                                    self.form_data["slocation-input"],
                                    id="slocation-input",
                                    validators=[PathExists(target_directory=True)],
                                    validate_on=["changed"],
                                )
                    ...
                    with Vertical(classes="form-option"):
                        with Horizontal():
                            ...
                            yield Input(
                                self.form_data["ssize-input"],
                                id="ssize-input",
                                type="integer",
                                validators=[
                                    Integer(4, psutil.disk_usage("/").free // 1_000_000)
                                ],
                                validate_on=["changed"],
                            )
            ...
            with Horizontal(id="options"):
                yield Button(
                    "Continue", variant="primary", id="continue-btn", disabled=True
                )
    ...
    async def select_directory(self, event: Button.Pressed):
        ...
        if str(event.button.label).startswith("Directory"):
            opened = await self.app.push_screen_wait(
                SelectDirectory(self.form_data[f"{attribute}-input"])
            )
        else:
            opened = await self.app.push_screen_wait(
                FileOpen(self.form_data[f"{attribute}-input"])
            )
        if opened:
    ...
    def update_slider(self, event: Slider.Changed):
        ...
        if slider_name == "memory":
            self.query_exactly_one("#" + slider_name + "-target", Static).update(
                f"Base Memory: {event.value} MB"
            )
        elif slider_name == "cpu":
            self.query_exactly_one("#" + slider_name + "-target", Static).update(
                f"CPU Cores: {event.value}"
            )
    ...
    def continue_step(self):
        ...
        # If all inputs are valid, then we either progress to next tab, or create the machine
        ...
        self.check_active_tab(
            self.query_exactly_one(TabbedContent).active_pane  # pyright: ignore [reportArgumentType]
        )  
    ...
    async def display_errors(self, event: Input.Changed):
        ...
        # Show any errors users make

        if event.validation_result is not None:
            err_element = self.query_exactly_one(
                "#error-" + event.input.id.split("-")[0], Static
            )
    ...
    def check_active_tab(
        self, active_tab: TabPane | Tab, event: Input.Changed | None = None
    ):
        # This function is responsible for checking how far the user is allowed to progress into the VM creation form
        ...
        if all(self.form_data[inp] for inp in tab_inputs):
            if next_tab != len(self.tab_form):
                self.query_exactly_one(
                    "#" + self.tab_form[next_tab][0], TabPane
                ).disabled = False
    ...
    def create_machine(self):
        ...
        settings_path = self._api.compose_machine_filename(
            self.form_data["name-input"], "/", "", self.form_data["parent-input"]
        )
        ...
        machine: Machine = self._api.create_machine(
            ...
            architecture,
            ["/"],
            "Linux26_64",  # OS detection not working at all sadly
            "",
            "",
            "",
            "",
        ...
        with machine.with_lock(save_settings=True, force_unlock=True) as mut_machine:
            ...
            progress = medium.create_base_storage(
                ((int(self.form_data["ssize-input"]) * 1_000_000) // 512) * 512,
                ["Standard"],
            )
            ...
            iso_medium = self._api.open_medium(
                self.form_data["iso-input"], "DVD", AccessMode.READ_ONLY, True
            )

            mut_machine.attach_medium(iso_medium, "IDE")  # Manually mount ISO because unattended installer wasn't working

            mut_machine.attach_medium(medium, "SATA")
